...
2. Create a new app for a remote MCP server and paste the URL: `https://crawler.insightfulmcp.com/`
...
```bash
claude mcp add --transport http crawler https://crawler.insightfulmcp.com/
```
...
```json
{
  "mcpServers": {
    "crawler": {
      "url": "https://crawler.insightfulmcp.com/"
    }
  }
}
```
...
```
"Check the robots.txt and indexability of https://example.com"
```
...
The Web Crawler MCP server is included in every InsightfulPipe plan, together with all other MCP servers and the CLI. Plans start at $29.99/month with a 7-day free trial. See [insightfulpipe.com/pricing](https://insightfulpipe.com/pricing) for current plans.
...
- [Documentation](https://insightfulpipe.com/docs/connectors-web-crawler)
